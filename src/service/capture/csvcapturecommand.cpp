...
void CsvCaptureCommand::execute()
{
    ...
    const QString defaultFileName = QString("%1_waveform_%2.csv").arg(modelName, timestamp);
    const QString defaultPath     = QDir(m_ctx.lastSaveDir).filePath(defaultFileName);

    const QString filePath = QFileDialog::getSaveFileName(
        nullptr,
        QObject::tr("Save Waveform Data"),
        defaultPath,
    ...
    QtConcurrent::run([oscilloscopePtr, captureInProgress, filePath, captureChannel]() {
        ...
        try {
            const QString scopeTempPath =
                QString("C:\\TekScope\\Waveforms\\wave_ch%1.csv").arg(captureChannel);

            const bool success = oscilloscopePtr->captureWaveformFileToHost(
                captureChannel, filePath, "CSV", scopeTempPath);
