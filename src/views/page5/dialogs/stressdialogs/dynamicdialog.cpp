...
//  Constructor
// ══════════════════════════════════════════════════════
DynamicDialog::DynamicDialog(const QStringList& inputOptions,
    ...
    , m_cfg(initCfg)
{
    setWindowTitle(QString("Row %1 — Dynamic Test").arg(seqNo));
    setStyleSheet(DYNAMIC_DLG_STYLE);
...
//  buildUI
// ══════════════════════════════════════════════════════
void DynamicDialog::buildUI(const QStringList& inputOptions,
                            const QStringList& dyloadOptions)
{
    ...
    auto* titleLbl  = new QLabel("Dynamic Test");
    titleLbl->setObjectName("titleLbl");
    headerRow->addWidget(titleLbl);
    headerRow->addStretch();

    auto* badge = new QLabel("DYNAMIC");
    ...
    // ── Dynamic Load 選項 ────────────────────────
    m_dyloadCombo = new QComboBox;
    m_dyloadCombo->addItem("— None —", -1);
    for (int i = 0; i < dyloadOptions.size(); ++i)
        m_dyloadCombo->addItem(
            QString("%1.  %2").arg(i + 1).arg(dyloadOptions[i]),
            i);
    form->addRow("Dy Load :", m_dyloadCombo);

    auto* dyloadPreview = new QLabel("—");
    dyloadPreview->setObjectName("previewLbl");
    form->addRow("", dyloadPreview);
    ...
    // ── 動態預覽 — Dy Load ───────────────────────
    connect(m_dyloadCombo, &QComboBox::currentIndexChanged,
            this, [this, dyloadOptions, dyloadPreview](int) {
                const int dataIdx = m_dyloadCombo->currentData().toInt();
                if (dataIdx < 0 || dataIdx >= dyloadOptions.size()) {
                    dyloadPreview->setText("—");
                    return;
                }
                dyloadPreview->setText(
                    QString("Dy Load row:  %1").arg(dyloadOptions[dataIdx]));
            });
...
void DynamicDialog::restoreFromCfg()
{
    ...
    const int dyloadIdx = m_cfg.value("dyload_index", -1).toInt();
    if (dyloadIdx >= 0) {
        for (int i = 0; i < m_dyloadCombo->count(); ++i) {
            if (m_dyloadCombo->itemData(i).toInt() == dyloadIdx) {
                m_dyloadCombo->setCurrentIndex(i);
                break;
            }
        }
    }
...
void DynamicDialog::saveConfig()
{
    ...
    const int dyloadIdx = m_dyloadCombo->currentData().toInt();
    const QString dyloadLbl = (dyloadIdx >= 0)
                                  ? m_dyloadCombo->currentText().section("  ", 1)
                                  : QString();
    m_cfg["dyload_index"] = dyloadIdx;
    m_cfg["dyload_label"] = dyloadLbl;
