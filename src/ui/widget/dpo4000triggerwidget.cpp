...
DPO4000TriggerWidget::DPO4000TriggerWidget(QWidget* parent)
    : QGroupBox(tr("Trigger"), parent)
{
    setupUI();
    m_controller = new DPO4000TriggerController(this, this);
}

DPO4000TriggerWidget::~DPO4000TriggerWidget()
{
    cleanup();
}

void DPO4000TriggerWidget::cleanup()
{
    if (m_controller) {
...
void DPO4000TriggerWidget::setupUI()
{
...
void DPO4000TriggerWidget::createControls()
{
    // Trigger Type
    ...
    m_cmbTrigSource->addItems({"CH1", "CH2", "CH3", "CH4"});
    ...
    m_autoTrigTarget->setObjectName("triggerTaget");   // 與 DPO7000 保持一致（含原始拼寫）
...
void DPO4000TriggerWidget::setupLayout()
{
